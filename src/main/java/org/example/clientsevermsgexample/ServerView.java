...
public class ServerView implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        // 1) start the server & accept one client
        new Thread(() -> {
            try {
                serverSocket = new ServerSocket(5555, 1, InetAddress.getByName("localhost"));
                socket = serverSocket.accept();
                dis = new DataInputStream(socket.getInputStream());
                dos = new DataOutputStream(socket.getOutputStream());

                // read incoming in a loop
                while (true) {
    ...
    private void addLabel(String msg, Pos alignment) {
        ...
        lbl.setStyle("-fx-background-color: lightblue; -fx-padding: 5; -fx-border-radius: 5;");
